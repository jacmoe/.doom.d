...
On my Manjoro system, that was accomplished by installing =emacs-nativecomp=:
#+begin_src bash
...
On Linux, there are likely to be a C compiler installed.
...
In order to read email with Emacs, install =offlineimap= and =notmuch= following the steps outlined in this article: [[https://www.maketecheasier.com/use-email-within-emacs/][How to Use Email within Emacs - Make Tech Easier]]
#+begin_src bash
pamac install offlinemap notmuch
#+end_src
