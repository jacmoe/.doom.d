...
To switch from insert state to command state, type: '\\[boon-set-command-state]'
...
>> Type: '\\[boon-set-command-state]' to switch to command state.
...
>> Type '\\[move-beginning-of-line]' to move to the beginning of a line.

>> Type '\\[move-end-of-line]' to move to the end of a line.
...
via 'x'.  (Standard commands are always available under 'C-x')
...
>> Type '\\[boon-set-command-state]' to go back to command state.
...
>> Type 'd \\<boon-select-map>\\[boon-select-with-spaces] \\[boon-select-word]\\<boon-command-map>' to remove the word and its surrounding spaces.
...
2. The region to surround.  This region is specified by the same
   language as the arguments to 'd' or 'SPC' commands.
