...
:END:
#+attr_html: :alt Quick word definition :title Quick word definition :width 100%
[[file:images/define-word.png]]
*** Dictionary server
:PROPERTIES:
:CUSTOM_ID: dictionaries-etc-dictionary-server
:END:
#+attr_html: :alt Dictionary server lookup :title Dictionary server lookup :width 100%
[[file:images/dictionary.png]]
*** Webster
:PROPERTIES:
:CUSTOM_ID: dictionaries-etc-webster
:END:
#+attr_html: :alt Look up word in Merriam-Webster :title Look up word in Merriam-Webster :width 100%
[[file:images/webster.png]]
*** Powerthesaurus
:PROPERTIES:
:CUSTOM_ID: dictionaries-etc-powertheraurus
:END:
#+attr_html: :alt Synonym lookup :title Synonym lookup :width 100%
[[file:images/synonyms.png]]
#+attr_html: :alt Sentences lookup :title Sentences lookup :width 100%
[[file:images/sentences.png]]
*** Prose linting
:PROPERTIES:
:CUSTOM_ID: dictionaries-etc-prose-linting
:END:
#+attr_html: :alt Prose linting with Vale :title Prose linting with Vale :width 100%
[[file:images/vale.png]]
*** Mermaid
:PROPERTIES:
:CUSTOM_ID: dictionaries-etc-mermaid
:END:
#+attr_html: :alt Mermaid script can be used to define diagrams in code blocks :title Mermaid script can be used to define diagrams in code blocks :width 100%
[[file:images/gantt.png]]
#+attr_html: :alt Gantt diagram generated by Mermaid script :title Gantt diagram generated by Mermaid script :width 100%
...
| C-x 8 ^ i   | î         |             |           |             |           |
Use =C-x 8 C-h= to see a list of available characters that can be typed this way. You might have to run Emacs without any costumizations, =emacs Q=, in order to be able to enter the command.
...
:END:
#+attr_html: :alt Track-table keeps track of words written per day :title Track-table keeps track of words written per day :width 100%
...
#+attr_html: :alt Pomodoro session :title Pomodoro session :width 100%
...
:PROPERTIES:
:CUSTOM_ID: problems-and-how-to-fix-them
:END:
*** Editing in Org-mode
:PROPERTIES:
:CUSTOM_ID: problems-and-how-to-fix-them-editing-in-org-mode
:END:
Often, when editing an org-mode document, you want to insert a new line or copy something, and you place the point at what appears to be the beginning of an element.
#+attr_html: :alt problem :title problem :width 100%
[[file:images/om-problem1.png]]
However, if we swicth to /text-mode/ (=M-x text-mode= or =M-x tm=) we see that the point is not actually where we thought it was:
#+attr_html: :alt problem :title problem :width 100%
[[file:images/om-problem2.png]]
Switching back to /org-mode/ (=M-x org-mode= or =M-x om=) how do we position the point at the right place, how do we insert a new line?
#+attr_html: :alt problem :title problem :width 100%
[[file:images/om-problem3.png]]
We could simply switch to /text-mode/ and perform the edit operation there, but we can do better by moving the point to the beginnind of line by pressing =u= in command mode:
#+attr_html: :alt problem :title problem :width 100%
[[file:images/om-problem4.png]]
Curiously, the point is now at the very end of the previous line, but we can take advantage of that.
All we have to do is insert a new line below the point by pressing =C-o=:
#+attr_html: :alt problem :title problem :width 100%
[[file:images/om-problem5.png]]
Move the point down a line, and we have successfully inserted a new line:
#+attr_html: :alt problem :title problem :width 100%
[[file:images/om-problem6.png]]

Because it is useful to be able to swicth between /org-mode/ and /text-mode/ there are created two aliases:
=text-mode= is aliased to =tm=, and =org-mode= is aliased to =om=.

That means we can quickly swich back and forth by =M-x tm= and =M-x om=.
