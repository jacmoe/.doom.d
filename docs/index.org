...
#+attr_html: :alt Bibliography source code :title Bibliography source code :width 100%
...
** Problems and how to fix them :noexport:
:PROPERTIES:
...
However, if we switch to /text-mode/ (=M-x text-mode= or =M-x tm=) we see that the point is not actually where we thought it was:
#+attr_html: :alt problem :title problem :width 100%
...
We could simply switch to /text-mode/ and perform the edit operation there, but we can do better by moving the point to the beginning of line by pressing =u= in command mode:
#+attr_html: :alt problem :title problem :width 100%
...
Because it is useful to be able to switch between /org-mode/ and /text-mode/ there are created two aliases:
=text-mode= is aliased to =tm=, and =org-mode= is aliased to =om=.

That means we can quickly switch back and forth by =M-x tm= and =M-x om=.
